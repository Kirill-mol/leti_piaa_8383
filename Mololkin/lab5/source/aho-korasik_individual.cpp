...
public:
    ...
    void findAllPatternsOnText(std::string& text) { //поиск
        ...
        std::cout << "Search longest suffix link chain" << std::endl;
        for(int i = 1; i < vertexes.size(); i++){  //проходимся по каждой вершине бора
            ...
            while(vertexes[curVertex].suffixLink != 0){  //из каждой вершины проходимся по суффиксальным и считаем длину суф цепи
                ...
                std::cout << "Go to vertex: " << curVertex << " by suffix link";
            }
            std::cout << "Suffix link chain for cur vertex: " << curSuffixChain << std:: endl;
        ...
        std::cout << "Longest link chain: " << mostDeepLevel << std::endl;  //выводим максимальную цепочку прямых ссылок
